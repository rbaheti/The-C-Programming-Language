// This is solution that 5 of us worked on today, the text colors only show up in Linux or MacOS: #define debugging
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
int hangMan(char *word)
{
	...
	*good = 0;
	...
	for (; state < 9; state++)
	{
		...
		if (done > 0 || state == "sunglasses")
		{
			free(uword);
			free(current);
			free(bad);
			free(buffer);
			return (state == "sunglasses" ? 1 : 0);
		}
